...
\begin{abstract}
We examine the kinematics of the gas in the environments of massive galaxies hosting quasars at 
$z\sim2$. We employ 112 projected quasar pairs to study the circumgalactic gas of the foreground 
quasars in absorption. The foreground quasars have precise redshift measurements, using 
emission-lines with uncertainty $\lesssim300\,{\rm km\,s^{-1}}$ and average offsets from the 
systemic redshift $\lesssim|100\,{\rm km\,s^{-1}}|$. We stack the background quasar spectra at the 
foreground quasar's systemic redshift. The mean absorption in \ion{C}{2}, \ion{C}{4}, and \ion{Mg}{2} show large velocity widths, and have velocity 
...
In a previous paper of the Qussars Probing Quasars series\citep[][QPQ8]{QPQ8}, we 
...
large-scale inflow around star-forming galaxies, or a Kaiser effect for gas on on 
...
\citep[][QPQ1]{QPQ1} to use the standard technique of absorption-line spectroscopy with background 
quasars. These quasar pairs 
...
(ii) characterize the dynamics of flows of gas around these massive system. We further describe 
an aspect of this experiment that offers a unique opportunity to study galactic-scale flows: we 
argue that, the anisotropic or intermittent radiation of the foreground quasars may break the 
...
$|100\,{\rm km\,s^{-1}}|$ or less. According to \citep{Shen+16}, the [\ion{O}{3}] emission-line 
redshifts have the smallest total scatter (due to intrinsic scatter and measurement error) of 
...
[\ion{O}{3}] using a larger but lower redshift sample. In QPQ8 \citep{QPQ8}, we quantified 
...
\ion{Mg}{2} from the systemic to these redshifts. We reached a final sample size of 112. 
...
hand, shows weaker absorption, and the Gaussian model has a centroid closer to zero. 
...
randomly sampling from the full sample 10000 times. We introduce a Gaussian absorption profile to 
model each bootstrap realization, and quote the standard deviation in the the bootstrap 
realizations to be the scatter in the centroid of the data. The scatters 
...
the distribution of the absorption centroids of the bootstrap realizations. We find that 98\% of 
the centroids are at positive velocities. We place a generous $3\sigma$ upper limit to the small 
offset of the centroid from $z_{\rm fg}$ for the \ion{C}{2} mean absorption at 
$\delta v<+451\,{\rm km\,s^{-1}}$. Given the large intrinsic scatter, we do not attempt to explore 
whether there exists relative asymmetry between \ion{C}{2} and \ion{C}{4} absorption due to 
...
the mean tack in Figure~\ref{fig:stack_z1}. The absorption is weaker than the $z\sim2$ main QPQ9 
sample. Gaussian absorption models fitted to the stack recover a centroid of 
$-13\pm410\,{\rm km\,s^{-1}}$ and a dispersion of $155\,{\rm km\,s^{-1}}$. The average offset 
from $0\,{\rm km\,s^{-1}}$ is smaller than the offsets in the $z\sim2$ sample. 

Since the large scatter in the centroids represent intrinsic variation rather than redshift 
errors, and the \ion{Mg}{2} stack for lower redshift does not suggest a similar offset, we 
...
Figure~\ref{fig:stacks_and_fits} is shown in thick, black. In thin, green, we overplot the 
...
absorption profile with $\sigma_v=658\,{\rm km\,s^{-1}}$. Motions that produce this width are 
ruled out at $3\sigma$ confidence by bootstrap analysis on the observational data. 
}
\label{fig:monte}
\end{figure}
...
redshifts, we recover $\sigma_{\rm intrinsic}^{\rm [OIII]}=342\,{\rm km\,s^{-1}}$. In QPQ8, we 
reported the line-of-sight velocity 
dispersion typical of QPQ dark matter halo mass is $\sigma_{\rm 1D}=212\,{\rm km\,s^{-1}}$. We can 
investigate whether gravitational motions and Hubble flows are sufficient to reproduce the 
dispersion in the \ion{C}{2} mean stack using Monte Carlo methods. 
...
equivalent width $W_{\rm CII}$ for \ion{C}{2} and a Gaussian absorption profile. We repeat the 
...
ampltiude of the best-fit Gaussian of the stack of mock spectra matches the 
amplitude of the stack of the observational data. We find that $W_{\rm CII}=0.7\,{\rm \AA}$ well 
reproduces the amplitude, and the dispersion of the Gaussian absorption model is 
insensitive to the assumed $W_{\rm CII}$ or line profile for one absorber. In 
...
The resulting stack of mock spectra has a $1\sigma$ dispersion of $244\,{\rm km\,s^{-1}}$. This is 
within $1\sigma$ modeling error of the intrinsic dispersion in the \ion{C}{2} mean stack for the 
full sample, and about 3 times larger than the modeling error of the intrinsic 
dispersion in the stack of the sub-sample with [\ion{O}{3}] redshifts. We also test for the 
...
the observed dispersion, a width of $\sigma_v>658\,{\rm km\,s^{-1}}$ is ruled out at 3$\sigma$ 
confidence (overplotted in Figure~\ref{fig:monte}). In other words, motions in additional to 
gravitational and Hubble flows that will produce $\sigma_v>611\,{\rm km\,s^{-1}}$ are ruled out. 
...
The stack for \ion{C}{4}, which include line-of-sight absorbers at all distances, shows a large, 
...
Figure~\ref{fig:stacks_fg}, we present mean stacks of foreground quasar spectra for the pairs in 
...
$120^\circ\textendash140^\circ$ may reproduce the observed centroid and intrinsic dispersion of 
the \ion{C}{2} mean absorption. The first explanation above to asymmetric absorption requires that 
the quasars emit their ionizing radiation anisotropically, while the second explanation requires 
the quasars emit their ionizing radiation intermittently. Both explanations will require the gas 
to be either outflowing or in Hubble flow. Were the gas inflowing instead, the velocity centroid 
would be negative. \cite{KirkmanTytler08} reported an asymmetry in \ion{H}{1} absorption on scales 
larger than the CGM, and gave similar arguments on anisotropy or intermittence. We are assembling 
a sample of quasar pairs with precise $z_{\rm fg}$ measurements to study this asymmetry in 
\ion{H}{1} (J. F. Hennawi et al. 2017, in preparation). 
...
either shine anisotropically or intermittently. 
