...
def experiment(outfil=None):
    ...
    for kk in np.arange(nplots):
        ...
        for ii,row in enumerate(subsample):
            sv_Rphys.append(row['R_PHYS'])
            sv_zfg.append(row['Z_FG'])
            sv_gUV.append(row['G_UV'])
            print(sv_gUV)
