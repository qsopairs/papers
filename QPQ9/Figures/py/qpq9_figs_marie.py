...
#            ispec_dicts = qpqs.load_spec(ispecfs)
            # Append
...
#            ax.set_xlabel(r'$R_\perp$ (kpc)')
        ax.set_xlabel(r'$R_\perp$ (kpc)')
        ax.text(10., 1.7, xai.ion_name(aline.data), size=40 )

        # Loop on QPQ9
        msk = qpq9['R_PHYS'] > qpq9['R_PHYS']
        for jj,iqpq9 in enumerate(qpq9):
            spec_files = qpqs.get_spec_files((iqpq9['RA']*u.degree,iqpq9['DEC']*u.degree))
            spec_dicts = []
            for ispecfs in spec_files:
                ispec_dicts = qpqs.load_spec(ispecfs)
                spec_dicts.append(ispec_dicts)
...
#            for s_dict in spec_dicts[jj]:
            for s_dict in spec_dicts:
                ...
                # Coverage?
                if sflg: 
                    if (line>wrest_mnx[0]) & (line<wrest_mnx[1]):
                        # Line
                        if not msk[jj]:
#                            ax.plot( [iqpq9['R_PHYS'].value]*2, zlim, '--', color='gray', alpha=0.5, lw=0.5)
        ...
        sc = ax.scatter( qpq9['R_PHYS'][msk].value, qpq9['FG_ZIR'][msk], s=psz,
                         c=np.log10(qpq9['G_UV'][msk]),edgecolors='none')
