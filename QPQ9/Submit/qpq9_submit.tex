...
observations, and have been analyzed separately in QPQ8 (see their Figure 8, 9, and 10). For this
...
$1\sigma$ modeling error for the centroid and dispersion of the \ion{C}{2} mean stack are 
...
skewed toward positive velocities. We have 33 pairs in the QPQ9 sample for \ion{C}{2} analysis
and an even larger number of pairs for \ion{C}{4} and \ion{Mg}{2} analyses, and the total number
...
of all absorbing components should tend toward Gaussian distribution. To model the absorption, we
introduce a Gaussian profile while allowing a constant pseudo-continuum level to vary. We perform
$\chi^2$ minimization with each channel given equal weight. From the best-fit to the data, we
measure the centroid of the \ion{C}{2} stack to be $+196\,{\rm km\,s^{-1}}$ and the $1\sigma$
dispersion to be $313\,{\rm km\,s^{-1}}$. The centroid suggests an asymmetry that contradicts the
standard expectation, while the dispersion suggests extreme kinematics. The median stack, on the
other hand, shows weaker absorption, and the Gaussian model has a centroid with smaller offset and
...
velocity structure and again shows a positive offset $\approx+259\,{\rm km\,s^{-1}}$. We thus
...
$\sigma_{\rm intrinsic}^{\rm full}=255\,{\rm km\,s^{-1}}$. For the sub-sample with [\ion{O}{3}] 
...
about $1\sigma$ modeling error away from the intrinsic dispersion in the \ion{C}{2} mean stack for 
the full sample, and about 3 times the modeling error away from the intrinsic dispersion in the 
stack of the sub-sample with [\ion{O}{3}] redshifts. We also test for the 
...
simulations may reproduce the observed centroid and intrinsic dispersion of the \ion{C}{2} mean
...
%intrinsic dispersion of the \ion{C}{2} mean absorption.
