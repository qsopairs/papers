...
$1\sigma$ modeling error for the centroid and dispersion of the \ion{C}{2} mean stack {\bf are
...
skewed toward positive velocities. {\bf We have 40 pairs in the QPQ9 sample for \ion{C}{2} analysis
...
to the data, we measure the centroid of the \ion{C}{2} stack {\bf to be \bf $+232\,{\rm km\,s^{-1}}$ and
...
absorption with Gaussian best-fit. The \ion{C}{2} mean stack for this sub-sample has a {\bf centroid at
...
limit to the small offset of the centroid from $z_{\rm fg}$ for the \ion{C}{2} mean absorption {\bf at
...
more symmetric \ion{Mg}{2} or H$\alpha$ emission when available. We are able to replace for {\bf 11 out
...
dominated by \ion{Mg}{2} emission, a shift of $+57\,{\rm km\,s^{-1}}$ is applied. There are {\bf 233}
...
for the \ion{C}{2} mean stack. {\bf 97\%} of the centroids are positively offset from $z_{\rm fg}$.}
...
about {\bf 2} times the modeling error away from the intrinsic dispersion in the \ion{C}{2} mean stack
for the full sample, and about {\bf 2} times the modeling error away from the intrinsic dispersion in the
stack of the sub-sample with [\ion{O}{3}] redshifts {\bf as well}. We also test for the
...
$\sim10^6$\,yr.
{\bf In turn, this implies a minimum quasar lifetime of $10^6$\,yr, which lies within existing
constraints from}
...
\ion{C}{4} absorption. The redshifted \ion{C}{4} may be regarded as an intermediate, and a
