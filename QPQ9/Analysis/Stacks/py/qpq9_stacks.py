...
def plt_qpq9(stack_tup=None,wrest=None,
    ...
    for ipage in range(pages):
        # Start the plot
        if outfil_prefix is None:
            outfil_prefix = 'plt_qpq_IRMgII'
        if stack_fg is True:
            outfil = outfil_prefix+'_{:d}_fg_page{:d}.pdf'.format(int(wrest.value),ipage+1)
        else:
            outfil = outfil_prefix+'_{:d}_page{:d}.pdf'.format(int(wrest.value),ipage+1)
        pp = PdfPages(outfil)
